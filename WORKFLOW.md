# Project Workflow for Forge

## 1. Overview
This document defines the collaboration protocol between the human user and the AI Agent for developing **Forge**. We treat the AI as a full partner in the engineering process.

## 2. Core Philosophy
*   **Agent as Engineer**: You are not just a chat bot; you are the **Scaffolding Architect**.
*   **Git-Ops Centric**: All changes must flow through Git branches (`feat/`, `fix/`, etc.).
*   **Discovery-Driven**: Decisions are based on the state of `reference_*` projects, not assumptions.
...
3.  Defines the "Why", "What" (Model/View/Controller changes), and "Verification Plan".
...
*   **Verification**: Always run `pytest` before declaring victory.
